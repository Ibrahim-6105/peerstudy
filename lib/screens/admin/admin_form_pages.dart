...
// AcademicAreaFormValue contains the validated values returned by the Area page.
class AcademicAreaFormValue {
  // The constructor makes every database value required and easy to inspect.
  const AcademicAreaFormValue({
    required this.code,
    required this.name,
  ...
  // These fields map directly to the academic_areas table.
  final String code;
  final String name;
  final String status;
...
class AcademicAreaFormPage extends StatefulWidget {
  // A null existing row means Add; a row means Edit.
  const AcademicAreaFormPage({super.key, this.existing});

  // Existing values are used only to prefill the controls.
  final AdminRow? existing;

  // Flutter creates the temporary form state here.
  @override
  State<AcademicAreaFormPage> createState() => _AcademicAreaFormPageState();
}

// This state owns only the values visible on the Area form.
class _AcademicAreaFormPageState extends State<AcademicAreaFormPage> {
  // FormState runs all beginner-friendly validators before returning a result.
  final GlobalKey<FormState> _formKey = GlobalKey<FormState>();

  // Controllers are created once and disposed with the page.
  late final TextEditingController _nameController;
  late final TextEditingController _orderController;

  // Dropdown choices remain ordinary local state updated with setState.
  late String _code;
  late String _status;

  // Prefill every field when the Admin is editing.
  @override
  void initState() {
    super.initState();
    final row = widget.existing;
    final savedCode = row?['code']?.toString() ?? 'IT';
    _code = const <String>{'IT', 'ENGINEERING'}.contains(savedCode)
        ? savedCode
        : 'IT';
  ...
  // Release text resources when the page closes.
  @override
  void dispose() {
  ...
  // Validate and return one simple value object to the dashboard.
  void _save() {
    FocusScope.of(context).unfocus();
    if (!(_formKey.currentState?.validate() ?? false)) return;
    Navigator.pop(
      context,
      AcademicAreaFormValue(
        code: _code,
  ...
  // Draw a calm page instead of placing all controls inside a popup dialog.
  @override
  Widget build(BuildContext context) {
    final isEditing = widget.existing != null;
    return _AdminFormFrame(
      title: isEditing ? 'Edit academic area' : 'Add academic area',
      intro: 'Set the area identity and where it appears in the catalog.',
      saveLabel: isEditing ? 'Save changes' : 'Add area',
      onSave: _save,
      child: Form(
        key: _formKey,
        ...
        child: Column(
          ...
          children: <Widget>[
            _FormSection(
              ...
              children: <Widget>[
                ...
                TextFormField(
                  ...
                  validator: (value) => _requiredName(value, 120),
                ),
              ],
            ),
            const SizedBox(height: 12),
            _FormSection(
              ...
              description: 'Inactive areas stay hidden from Students.',
              children: <Widget>[
